function results = compare_centers_centroid_union(target, varargin)
...
%> Dthresh, matching transinfoUnion's own domain definition) contribute. It
%then plots all three centers on top of each image and reports the
%pairwise distances between them.
%
%This is the transinfoUnion variant of compare_centers_centroid.m: TI is
%computed with transinfoUnion.m, which pads the original and transformed
%images out to the union of their domains (rather than intersecting them)
%and sets non-overlapping padding to 1 (see transinfoUnion.m), so there is
%no shrinking-intersection failure mode and no coverage output to mask on.
%
%   results = compare_centers_centroid_union()
%   results = compare_centers_centroid_union(target)
%   results = compare_centers_centroid_union(target, 'Name', Value, ...)
%
...
%   compare_centers_centroid_union('angiosperms', 'MaxImages', 5, ...
...
if iscell(target) || (isstring(target) && ~isscalar(target))
    ...
    elseif isfolder(targetPath)
        ...
        error('compare_centers_centroid_union:badTarget', 'Could not find image or folder: %s', target);
    end
end
...
if ~isfile(tp)
    error('compare_centers_centroid_union:badTarget', 'Could not find image file: %s', t);
end
d = dir(tp);
entry = d(1);
end
...
pyc = ceil(M/2);
pxc = ceil(N/2);
xcoord = (pxc-floor(N/4)):(pxc+ceil(N/4));
ycoord = (pyc-floor(M/4)):(pyc+ceil(M/4));
...
for llx = 1:NXmax
    ...
    for kky = 1:NYmax
        ...
        for mth = 1:length(thetavals)
            ...
            TIcenter(llx,kky,mth) = transinfoUnion(imdata, Atform, Dthresh, Pmax);
        end
    end
end
...
for llx = 1:NXmax
    ...
    TIx(llx) = transinfoUnion(imdata, Atb*Arx*AtbI, Dthresh, Pmax);
end
...
for kky = 1:NYmax
    ...
    TIy(kky) = transinfoUnion(imdata, Atb*Ary*AtbI, Dthresh, Pmax);
end

bxc = xshifts(min_with_endpoints(TIx));
byc = yshifts(min_with_endpoints(TIy));
end


function idx = min_with_endpoints(TIvec)
...
[pks, locs] = findpeaks(-TIvec);
pks = horzcat(TIvec(1), -pks, TIvec(end));
locs = horzcat(1, locs, length(TIvec));
[~, n] = min(pks);
idx = locs(n);
end
...
%intensity > Dthresh, matching transinfoUnion's own domain definition) is
